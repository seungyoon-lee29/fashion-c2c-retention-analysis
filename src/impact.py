...
Three estimators of the SAME estimand (risk difference in converting by horizon H
...
from personperiod import (CONFOUNDER_COLS, CONVERT, build_person_period,
                          early_features)
from survival import DESIGN_COLS, empirical_cif, fit_cause_specific, predict_hazards
...
def gformula_cif(bundle: dict, feats: pd.DataFrame, x: int, cfg: dict) -> float:
    """Population CIF of conversion by horizon under do(aha_cart=x), standardising over features."""
...
def naive_delta(pp: pd.DataFrame) -> tuple[float, float, float]:
    lab = pp.groupby("user_id").agg(conv=("event", lambda s: int((s == CONVERT).any())),
                                    aha=("aha_cart", "first"))
...
def iptw_delta(pp: pd.DataFrame, feats: pd.DataFrame, cfg: dict) -> dict:
    lab = pp.groupby("user_id").agg(conv=("event", lambda s: int((s == CONVERT).any()))).reset_index()
...
def markov_absorbing(pp: pd.DataFrame, cfg: dict) -> dict:
    ...
    b_convert = N * h_c                     # absorption prob into conversion
    ...
    return {"per1000_point": float(1000 * b_convert),
...
def run(cfg: dict, events, cohort, feats=None, pp=None) -> dict:
    ...
    res = {
        ...
        "markov": mk, "ledger": ledger, "disagreement": diverge,
    }
...
def _write_report(r: dict, cfg: dict):
    ...
    L.append("## Conversion risk difference under the activation lever (by horizon)\n")
    ...
    mk = r["markov"]
    L.append(f"- expected conversions per 1000 (eventual): **{mk['per1000_point']:.0f}** "
